...
def sensitivity_avoidance():
    ...
    scenarios = {
        ...
        "D 제외 3변수 (0.35/0.35/0.30)": {"A_도움부재_s": 0.35, "B_외로움부정_s": 0.35, "C_복지불신_s": 0.30, "D_네트워크축소_s": 0.00},
    }
...
def save_report(dep_df, avo_df):
    ...
    # D 제외 3변수 시나리오 rho 추출
    _3var_rows = avo_df[avo_df["시나리오"].str.startswith("D 제외")]
    three_var_rho = _3var_rows["Spearman_rho"].iloc[0] if len(_3var_rows) > 0 else float("nan")
...
**결론**: 대체로 유지되지만, 복지불신 비중을 극단적으로 높이면(0.40) rho=0.889로 민감해짐. 자치구 수(N=25)가 적어 Dependency보다 가중치에 더 민감함. D 제외 3변수 버전({three_var_rho:.4f})은 D의 PQ47 계열 중복이 결론에 미치는 영향을 확인함.
...
- **D 제외 3변수**: D(네트워크축소)가 A(도움부재)와 동일 PQ47 계열에서 파생되는 중복을 제거한 버전. rho={three_var_rho:.4f}로 {"D 포함 여부가 결론에 영향 없음 확인됨" if three_var_rho > 0.95 else "D 변수가 순위에 유의미한 영향을 미침"}.
